...
const PostDetail = () => {
  ...
  // 상황별 정렬 (항상 OOPS, OVERCOMING, OVERCOME 순)
  const orderedPosts =
    postDetail?.postGroup
      ?.slice() // 불변성 유지를 위해 복사
      .sort(
        (a, b) =>
          SITUATION_ORDER.indexOf(a.situation as any) -
          SITUATION_ORDER.indexOf(b.situation as any)
      ) || [];
    ...
    <>
      <div className="w-full flex flex-col bg-[#fffbf8]">
        ...
        <section className="w-full ">
          ...
          >
            {SITUATION_ORDER.map((state, index) => (
              <SwiperSlide
                key={state}
